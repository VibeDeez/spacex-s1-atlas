...
async function staticChecks() {
  ...
  const staleDomains = ['jade-serenity-vhqv.here.now', 'brave-fennel-ghck.here.now', 'sentient-ravine-9vxe.here.now', 'mossy-petal-6adt.here.now', 'stellar-nirvana-tvwp.here.now']
  ...
  return { jsAsset: jsAssets[0], cssAsset: cssAssets[0], atlasCount: atlas.length, sharePageCount: sharePages.length, riskCount: risks.length }
}
...
async function httpChecks(origin) {
  const required = ['summary.json', 'financials.json', 'risks.json', 'atlas-index.json', 'debate-lenses.json', 'source-main.json', 'source-exhibits.json', 'ocr.json', 'social/spacex-s1-atlas-card.png', 'share/risk-radar/']
