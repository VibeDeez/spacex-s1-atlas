  ...
  return (
    <header className="sticky top-0 z-50 border-b border-white/[0.075] bg-void/94 pt-safe backdrop-blur-md sm:backdrop-blur-2xl">
      <div className="mx-auto flex max-w-[1760px] items-center gap-2 px-safe py-2 sm:gap-3">
        <button onClick={() => setHash('/flight-deck')} className="flex shrink-0 items-center gap-3 rounded-xl px-2 py-1 text-left hover:bg-white/[0.035] focus-ring">
...
function Source({ children }) {
  return (
    <p className="source-line card-surface-soft mt-2 max-w-full rounded-md border px-2 py-1 font-mono text-[11px] leading-4 text-white/72">
      <span className="shrink-0 whitespace-nowrap font-semibold uppercase tracking-normal text-cyan/80">SRC</span>
...
function SourceChip({ children, label = 'Source' }) {
  ...
  return (
    <span title={String(children)} className="source-chip card-surface-soft mt-2 inline-flex max-w-full items-center gap-1 rounded-md border px-2 py-1 font-mono text-[10px] uppercase tracking-normal text-white/62">
      <span className="shrink-0 whitespace-nowrap text-cyan/76">{label}</span> <span className="shrink-0 text-white/58">·</span> <span className="min-w-0 truncate normal-case tracking-normal text-white/70">{trimText(children, 74)}</span>
    </span>
  )
}
      ...
      <div className={cn(mobileCard && 'hidden md:block')}>
        <div className={cn('overflow-auto overscroll-x-contain', maxHeight)}>
          <table aria-label={ariaLabel} className="data-table min-w-full border-collapse text-left text-[13.5px]">
            <thead className="sticky top-0 z-10 bg-panel2 text-[11px] uppercase tracking-normal text-white/68">
...
function ShareActions({ packet, data, variant = 'business-stack', title, source, compact = false }) {
  ...
  return (
    <div className={cn('flex flex-wrap gap-2', compact && 'text-xs')}>
      <button onClick={() => shareToDevice({ title: label, caption, url, setCopied })} className="inline-flex min-h-12 flex-1 items-center justify-center gap-2 rounded-xl bg-spacex px-4 text-sm font-[650] text-void hover:bg-white focus-ring sm:flex-none"><Share2 size={15} /> Share</button>
      <button onClick={() => copyText(url, setCopied)} className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-white/10 bg-white/[0.035] px-3 text-xs font-[560] text-white/74 hover:bg-white/[0.06] focus-ring"><LinkIcon size={14} /> Copy link</button>
      <button onClick={() => copyText(caption, setCopied)} className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-white/10 bg-white/[0.035] px-3 text-xs font-[560] text-white/74 hover:bg-white/[0.06] focus-ring"><Clipboard size={14} /> Copy caption</button>
      <button onClick={async () => { try { await exportPoster({ variant, data, packet, share: true }) } catch { await copyText(caption, setCopied) } }} className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-white/10 bg-white/[0.035] px-3 text-xs font-[560] text-white/74 hover:bg-white/[0.06] focus-ring"><Download size={14} /> PNG</button>
  ...
  return (
    <Panel pad="p-4" className="relative overflow-hidden">
      ...
      {related.length > 0 && (
        <div className="mt-3 rounded-xl border border-white/[0.07] bg-white/[0.018] p-3">
          ...
          <div className="mt-2 grid gap-2">
            {related.map((item) => (
              <button key={item.id} onClick={() => setHash(item.hash)} className="rounded-lg border border-white/[0.06] bg-black/20 px-3 py-2 text-left text-xs leading-5 text-white/70 hover:bg-white/[0.045] focus-ring">
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <button onClick={() => setHash(packet.hash)} className="inline-flex min-h-11 items-center gap-2 rounded-lg bg-spacex px-3 text-xs font-[650] text-void hover:bg-white focus-ring">Open packet <ChevronRight size={14} /></button>
  ...
  return (
    <div className="rounded-lg border border-white/[0.08] bg-black/20 p-3">
      ...
      <div className="grid gap-2">
        {cards.map((fact) => (
          <button key={fact.k} onClick={() => setHash('/atlas', { q: fact.k, type: 'fact' })} className="rounded-lg border border-white/[0.07] bg-white/[0.025] p-3 text-left transition hover:border-white/16 hover:bg-white/[0.045] focus-ring">
  ...
  return (
    <Panel pad="p-4 sm:p-5" className="card-surface-elevated h-full">
      ...
      <div className="mt-4 divide-y divide-white/[0.075]">
        {rows.map((row) => (
          <button key={row.label} onClick={() => setHash(row.label.includes('Governance') ? '/governance' : row.label.includes('Segment') || row.label.includes('Financial') || row.label.includes('Profitability') || row.label.includes('Liquidity') ? '/financials' : '/atlas')} className="grid w-full gap-2 py-3 text-left transition hover:bg-white/[0.025] focus-ring sm:grid-cols-[10rem_minmax(0,1fr)] sm:px-2">
      ...
      <Panel pad="p-4 sm:p-5" className="card-surface-elevated memo-panel overflow-hidden">
        <div className="grid gap-4 xl:grid-cols-[minmax(0,.9fr)_minmax(500px,1.1fr)]">
          <div className="min-w-0">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              <button onClick={() => setHash('/financials')} className="inline-flex min-h-11 items-center gap-2 rounded-lg bg-spacex px-4 text-sm font-[700] text-void hover:bg-white focus-ring">Open financial workbench <ChevronRight size={15} /></button>
              <button onClick={() => scrollToId('question-tree')} className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-white/12 bg-white/[0.04] px-4 text-sm font-[620] text-white/78 hover:bg-white/[0.065] focus-ring"><FileSearch size={15} /> Open question tree</button>
              <button onClick={() => setHash('/atlas')} className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-white/12 bg-white/[0.03] px-4 text-sm font-[620] text-white/72 hover:bg-white/[0.06] focus-ring"><Search size={15} /> Open source atlas</button>
            </div>
          </div>
          <div className="card-surface-soft overflow-hidden rounded-lg border">
            {proofPoints.map((point) => (
              <button key={point.label} onClick={() => setHash(point.label === 'Control and blanks' ? '/governance' : point.label === 'Segment split' ? '/segments' : '/financials')} className="grid w-full gap-2 border-b border-white/[0.055] p-3 text-left transition last:border-b-0 hover:bg-white/[0.035] focus-ring sm:grid-cols-[8.5rem_minmax(0,1fr)]">
                <span>
...
function ThesisAntiThesisPanel({ data }) {
  ...
  const Column = ({ title, rows, tone }) => (
    <div className="card-surface-soft rounded-lg border p-3">
      ...
      <div className="mt-3 divide-y divide-white/[0.065]">
        {rows.map((row) => (
          <button key={row.title} onClick={() => setHash(row.route)} className="block w-full py-3 text-left first:pt-0 last:pb-0 hover:bg-white/[0.02] focus-ring">
      ...
      <Panel pad="p-0" className="overflow-hidden">
        <div className="grid gap-2 p-2 md:hidden">
          {ordered.map((row) => {
            ...
            return (
              <button key={row.segment} onClick={() => packet ? onPacket?.(packet) : setHash('/financials')} className="card-surface-soft rounded-lg border p-3 text-left focus-ring">
                <div className="flex items-start justify-between gap-3">
                  <div>
        ...
        <div className="hidden overflow-auto overscroll-x-contain md:block">
          <table className="analyst-table min-w-[900px] w-full border-collapse text-left">
            <thead className="bg-panel2 font-mono uppercase tracking-normal">
              <tr>
                ...
                return (
                  <tr key={row.segment} className="hover:bg-white/[0.025]">
                    <td className="sticky-matrix-col px-3 py-3 align-top">
                      <button onClick={() => packet ? onPacket?.(packet) : setHash('/financials')} className="text-left focus-ring">
    ...
    <Section id="question-tree" dense eyebrow="Question tree" title="Start with the investor question, then open the evidence.">
      <div id="question-tree" className="grid gap-3 lg:grid-cols-[.62fr_1.38fr]">
        <Panel pad="p-4" className="self-start border-spacex/14">
          ...
          <button onClick={() => setHash('/debate')} className="mt-4 inline-flex min-h-11 items-center gap-2 rounded-lg border border-white/10 bg-white/[0.035] px-3 text-xs font-[620] text-white/72 hover:bg-white/[0.06] focus-ring">Open full Debate Map <ChevronRight size={14} /></button>
        </Panel>
        <div className="grid gap-2">
          {questions.map((item, idx) => (
            <button key={item.id} onClick={() => setHash(item.route)} className="grid gap-3 rounded-lg border border-white/[0.08] bg-black/18 p-3 text-left transition hover:border-white/18 hover:bg-white/[0.04] focus-ring md:grid-cols-[4rem_minmax(0,1fr)_auto] md:items-start">
  ...
  return (
    <Section dense eyebrow="Risk / control map" title="Start with the operating dependencies; use the radar only to locate exact risk language.">
      <Panel pad="p-0" className="overflow-hidden">
        <div className="grid gap-2 p-2 md:hidden">
          {rows.map((row, idx) => (
            <button key={row.dependency} onClick={() => row.packet ? setHash(row.packet.hash) : setHash('/risks')} className="rounded-lg border border-white/[0.075] bg-white/[0.026] p-3 text-left focus-ring">
            ...
            <tbody className="divide-y divide-white/[0.06]">
              {rows.map((row) => (
                <tr key={row.dependency} className="hover:bg-white/[0.025]">
                  ...
                  <td className="px-3 py-3 align-top">
                    {row.packet ? (
                      <button onClick={() => setHash(row.packet.hash)} className="inline-flex min-h-9 items-center gap-1 rounded-md border border-white/[0.08] bg-white/[0.03] px-2 text-[11px] text-white/68 hover:bg-white/[0.06] focus-ring">Open packet <ChevronRight size={12} /></button>
                    ) : (
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 border-t border-white/[0.07] px-3 py-3">
          ...
          <button onClick={() => setHash('/risks')} className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-red/20 bg-red/10 px-3 text-xs font-[620] text-white/78 hover:bg-red/15 focus-ring"><Radar size={14} /> Open visual Risk Radar</button>
        </div>
      </Panel>
    </Section>
  )
...
function EvidenceDrilldown({ data, packet, onPacket }) {
  return (
    <Section dense eyebrow="Evidence workbench" title="Packets, source coverage, and share tools stay below the memo spine.">
      <details className="group rounded-lg border border-white/[0.08] card-surface-soft">
        <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-3 py-3 focus-ring">
          <span>
        ...
        <div className="grid gap-3 border-t border-white/[0.07] p-3 xl:grid-cols-[minmax(0,1fr)_420px]">
          <div className="grid gap-3 lg:grid-cols-2">
            ...
            <Panel pad="p-4" className="self-start">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <button onClick={() => setHash('/sources')} className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-white/10 bg-white/[0.035] px-3 text-xs font-[620] text-white/76 hover:bg-white/[0.06] focus-ring"><Database size={14} /> Full source text</button>
                <button onClick={() => setHash('/poster/business-stack')} className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-white/10 bg-white/[0.035] px-3 text-xs font-[620] text-white/76 hover:bg-white/[0.06] focus-ring"><Sparkles size={14} /> Poster / share</button>
              </div>
        ...
        <Panel pad="p-5 sm:p-7" className="relative overflow-hidden border-spacex/18">
          ...
          <div className="relative">
            ...
            <div className="mt-6 flex flex-wrap gap-2">
              <button onClick={() => setHash('/financials')} className="inline-flex min-h-12 items-center gap-2 rounded-lg bg-spacex px-4 text-sm font-[700] text-void hover:bg-white focus-ring">Open financial workbench <ChevronRight size={15} /></button>
              <button onClick={() => setHash('/risks')} className="inline-flex min-h-12 items-center gap-2 rounded-lg border border-white/12 bg-white/[0.045] px-4 text-sm font-[620] text-white/82 hover:bg-white/[0.07] focus-ring"><ShieldAlert size={15} /> Risk radar</button>
              <button onClick={() => setHash('/atlas')} className="inline-flex min-h-12 items-center gap-2 rounded-lg border border-white/12 bg-white/[0.035] px-4 text-sm font-[620] text-white/76 hover:bg-white/[0.065] focus-ring"><Search size={15} /> Search packets</button>
            </div>
    ...
    <Panel pad="p-4" className="overflow-hidden">
      ...
      <div className="grid gap-2 xl:grid-cols-3">
        {q1Segments.map((segment) => {
          ...
          return (
            <button key={segment.segment} onClick={() => packet ? onPacket?.(packet) : undefined} className="rounded-lg border border-white/[0.09] bg-white/[0.03] p-4 text-left transition hover:border-white/20 hover:bg-white/[0.055] focus-ring">
          ...
          <Panel key={card.title} pad="p-4" className="grid gap-3 lg:grid-cols-[minmax(0,.72fr)_minmax(0,1fr)]">
            ...
            <div>
              ...
              <div className="mt-2 flex flex-wrap items-center gap-2">
                ...
                {card.packets.filter(Boolean).map((packet) => <button key={packet.id} onClick={() => setHash(packet.hash)} className="min-h-8 rounded-md border border-white/[0.09] bg-white/[0.04] px-2 text-[11px] text-white/72 hover:bg-white/[0.07] focus-ring">Open packet</button>)}
              </div>
            </div>
          </Panel>
        ))}
  ...
  return (
    <div className="grid gap-3 2xl:grid-cols-3">
      {data.segments.map((segment) => {
        ...
        return (
          <button key={segment.name} onClick={() => packet && onPacket?.(packet)} className="card-surface-soft group relative min-h-[250px] overflow-hidden rounded-xl border p-4 text-left transition hover:border-white/18 hover:bg-white/[0.035] focus-ring">
          ...
          {counts.map((c) => {
            ...
            return (
              <div key={c.theme} className="rounded-xl border border-white/[0.075] bg-white/[0.025] p-3">
                ...
                <button onClick={() => setHash('/risks', { group: c.theme })} className="mt-3 rounded-lg border border-white/[0.08] bg-white/[0.03] px-3 py-2 text-xs font-[560] text-white/72 hover:bg-white/[0.06] focus-ring">Open all {c.count}</button>
              </div>
            )
          })}
  ...
  return (
    <div className="relative h-[148px] overflow-hidden rounded-xl border border-white/[0.07] bg-black/30 p-3">
      ...
      <div className="relative z-10 mt-4 grid grid-cols-3 gap-2">
        {segments.map((segment, idx) => (
          <div key={segment.name} className="rounded-lg border border-white/[0.07] card-surface-soft p-2 text-center">
    ...
    <section className="px-safe py-3 md:hidden">
      <Panel pad="p-4" className="relative overflow-hidden">
        ...
        <div className="mt-3 grid gap-2">
          <button onClick={() => setHash('/financials')} className="rounded-lg border border-amber/20 bg-amber/10 p-3 text-left focus-ring">
        ...
        <button onClick={() => setHash('/risks')} className="mt-3 flex min-h-12 w-full items-center justify-between rounded-xl border border-red/25 bg-red/10 px-3 text-left focus-ring">
        ...
        {packet && (
          <button onClick={() => setHash(packet.hash)} className="mt-2 block min-h-14 w-full rounded-xl border border-white/[0.08] bg-white/[0.025] px-3 py-2 text-left focus-ring">
    ...
    <Section dense eyebrow="Disclosure atlas" title="Search, filter and share every packet" aside="This is the dense workbench. Each row is now a stable disclosure packet with a deep link, caption and exportable card.">
      <div className="grid gap-3 xl:grid-cols-[300px_1fr_440px]">
        ...
        <DataTable maxHeight="max-h-[760px]" empty="No matching disclosures. Try a broader term or clear the type filter." columns={[
          ...
          { key: 'title', label: `Packets (${filtered.length})`, render: (r) => <button onClick={() => setHash(r.hash)} className="block max-w-4xl text-left text-[13px] font-[560] leading-5 text-white/82 hover:text-spacex focus-ring">{r.title}</button> },
          ...
          <button onClick={() => setHash(r.hash)} className="block min-h-20 w-full rounded-lg border border-white/[0.07] bg-white/[0.025] p-3 text-left focus-ring">
...
function LensChip({ children, active = false, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={cn('min-h-11 rounded-full border px-3 text-xs font-[580] transition focus-ring', active ? 'border-spacex/40 bg-spacex/12 text-spacex shadow-[0_0_26px_rgba(183,216,255,.08)]' : 'border-white/[0.085] bg-white/[0.025] text-white/68 hover:bg-white/[0.05]')}
    >
      {children}
    </button>
  )
  ...
  return (
    <div className="rounded-xl border border-white/[0.075] bg-black/24 p-3">
      <div className="flex items-start justify-between gap-3">
        ...
        {query && <button onClick={() => setHash('/atlas', { q: query })} className="shrink-0 rounded-lg border border-white/10 bg-white/[0.035] px-2 py-1.5 text-[10px] font-[560] text-white/62 hover:bg-white/[0.06] focus-ring">Atlas</button>}
      </div>
      <p className="mt-2 text-xs leading-5 text-white/68">{detail}</p>
      <Source>{source || packet?.source}</Source>
      {packet && <button onClick={() => setHash(packet.hash)} className="mt-2 inline-flex min-h-9 items-center gap-1 rounded-lg border border-white/10 bg-white/[0.03] px-2 text-[11px] text-white/64 hover:bg-white/[0.06] focus-ring"><FileSearch size={13} /> Open packet</button>}
    </div>
  )
}

function formatTufteValue(value, unit = 'number') {
  if (!hasNumber(value)) return '—'
  const n = Number(value)
  if (unit === 'money') return money(n)
  if (unit === 'tam') return n >= 1000 ? `$${(n / 1000).toFixed(n >= 10000 ? 1 : 2)}T` : `$${number(n)}B`
  if (unit === 'millions') return `${n.toFixed(n >= 10 ? 1 : 2)}M`
  if (unit === 'arpu') return `$${n.toFixed(0)}`
  if (unit === 'tons') return `${number(n)}t`
  if (unit === 'gw') return `${n.toFixed(1)} GW`
  if (unit === 'pct') return formatPlainPct(n, 1)
  return number(n)
}
...
function TufteBars({ rows, unit = 'money' }) {
...
function TufteReadouts({ data }) {
  ...
  const tamRows = (data.kubinModel?.tam?.segments || []).map((row) => ({
    label: row.Segment,
    value: row['TAM ($B)'],
    detail: `${(Number(row['% of Quantified TAM']) * 100).toFixed(1)}%`,
    color: SEGMENT_COLORS[row.Segment] || '#B7D8FF',
  }))
      ...
      <Panel pad="p-3 sm:p-4" className="overflow-hidden border-spacex/15 bg-[radial-gradient(circle_at_15%_20%,rgba(183,216,255,.13),transparent_34%),linear-gradient(135deg,rgba(255,255,255,.045),rgba(255,255,255,.012))]">
        <div className="grid gap-4 xl:grid-cols-[.82fr_1.18fr]">
          ...
          <div className="grid content-start gap-2 sm:grid-cols-2">
            {themes.map((theme) => (
              <button key={theme.id} onClick={() => setActive(theme.id)} className={cn('min-h-28 rounded-xl border p-3 text-left transition focus-ring', active === theme.id ? 'border-cyan/32 bg-cyan/10' : 'border-white/[0.075] bg-black/24 hover:bg-white/[0.04]')}>
            ...
            <div key={period} className="rounded-xl border border-white/[0.075] bg-black/24 p-3">
              ...
              <div className="mt-3 grid gap-3">
                {rows.map((row) => {
                  ...
                  const capexMix = margin(row.capex, totalCapex)
                  return (
                    <button key={`${period}-${row.segment}`} onClick={() => setHash('/financials')} className="rounded-xl border border-white/[0.065] bg-white/[0.025] p-3 text-left hover:bg-white/[0.045] focus-ring">
                      <div className="flex items-start justify-between gap-3">
        ...
        <div className="rounded-xl border border-white/[0.075] bg-black/24 p-3">
          ...
          <div className="mt-3 grid gap-2">
            {data.related?.map((party) => (
              <button key={party.party} onClick={() => setHash('/atlas', { q: party.party })} className="rounded-xl border border-white/[0.065] bg-white/[0.025] p-3 text-left hover:bg-white/[0.045] focus-ring">
        ...
        <div className="rounded-xl border border-red/18 bg-red/10 p-3">
          ...
          <div className="mt-3 grid gap-2">
            {data.risks?.filter((risk) => /controlled company|dual class|shareholders|forum|arbitration|proposal|Class B/i.test(risk.heading)).slice(0, 6).map((risk) => (
              <button key={risk.heading} onClick={() => setHash('/risks', { q: risk.heading.slice(0, 42) })} className="rounded-lg border border-white/[0.06] bg-black/25 p-2 text-left text-xs leading-5 text-white/68 hover:bg-white/[0.04] focus-ring">{trimText(risk.heading, 145)}</button>
            ))}
          </div>
        </div>
      </div>
  ...
  return (
    <Panel pad="p-3 sm:p-4">
      ...
      <div className="grid gap-3 xl:grid-cols-[1.05fr_.95fr]">
        <div className="grid gap-2 sm:grid-cols-2">
          {treasury.map((item) => (
            <button key={item.label} onClick={() => setHash('/atlas', { q: item.label.includes('Bitcoin') ? 'digital assets' : item.label })} className="rounded-xl border border-white/[0.075] bg-black/24 p-3 text-left hover:bg-white/[0.045] focus-ring">
              <div className="flex items-start justify-between gap-3">
    ...
    <Panel pad="p-3 sm:p-4">
      ...
      <div className="grid gap-3 lg:grid-cols-2 xl:grid-cols-5">
        {(data.debate?.holderPersonas || []).map((holder) => (
          <button key={holder.persona} onClick={() => setHash('/atlas', { q: holder.lookAt[0] })} className="rounded-xl border border-white/[0.075] bg-black/24 p-3 text-left hover:bg-white/[0.045] focus-ring">
    ...
    <Panel pad="p-3 sm:p-4">
      ...
      <div className="grid gap-3 xl:grid-cols-[.8fr_1.2fr]">
        <div className="rounded-xl border border-white/[0.075] bg-black/24 p-3">
          ...
          <button onClick={() => setHash('/risks', { q: claim?.label })} className="mt-3 min-h-11 rounded-lg border border-white/10 bg-white/[0.035] px-3 text-xs font-[560] text-white/70 hover:bg-white/[0.06] focus-ring">Open Risk Radar</button>
        </div>
        <div className="grid gap-2">
          {matches.map((risk) => (
            <button key={risk.heading} onClick={() => setHash('/risks', { q: risk.heading.slice(0, 64) })} className="rounded-xl border border-white/[0.07] bg-white/[0.025] p-3 text-left hover:bg-white/[0.045] focus-ring">
    ...
    <>
      ...
      >
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {themes.map((theme) => (
            <button key={theme.id} onClick={() => setActive(theme.id)} className={cn('rounded-xl border p-3 text-left transition focus-ring', active === theme.id ? 'border-spacex/32 bg-spacex/10' : 'border-white/[0.075] bg-black/24 hover:bg-white/[0.045]')}>
              <p className="font-mono text-[9px] uppercase tracking-normal text-white/58">{theme.label}</p>
...
function FinancialBasisBanner({ data, onInspect }) {
  ...
  return (
    <Panel pad="p-3" className="card-surface-elevated">
      <div className="grid gap-3 lg:grid-cols-[1.1fr_.9fr_.8fr]">
        {[{
          ...
          <button key={item.label} onClick={() => onInspect(inspectPayload({ title: item.title, value: item.label, formula: 'Filed disclosure; no arithmetic', source: item.source, inputs: [item.body] }))} className="rounded-xl border border-white/[0.075] bg-black/20 p-3 text-left hover:bg-white/[0.035] focus-ring">
            <p className="font-mono text-[9px] uppercase tracking-normal text-white/58">{item.label}</p>
    ...
    <Panel pad="p-3">
      <div className="mb-3 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          {periods.map((p) => <button key={p} onClick={() => setPeriod(p)} className={cn('min-h-10 rounded-lg border px-3 text-xs font-[560] focus-ring', period === p ? 'border-spacex/35 bg-spacex/10 text-spacex' : 'border-white/[0.08] bg-white/[0.025] text-white/60 hover:bg-white/[0.045]')}>{p}</button>)}
        </div>
      </div>
    ...
    <Panel pad="p-3">
      <div className="mb-3 flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
        ...
        <div className="no-scrollbar flex gap-2 overflow-x-auto">
          {lenses.map((item) => <button key={item.id} onClick={() => setLens(item.id)} className={cn('min-h-10 shrink-0 rounded-lg border px-3 text-xs focus-ring', lens === item.id ? 'border-amber/35 bg-amber/10 text-amber' : 'border-white/[0.08] bg-white/[0.025] text-white/58 hover:bg-white/[0.04]')}>{item.label}</button>)}
        </div>
      </div>
      <div className="overflow-auto rounded-xl border border-white/[0.075] bg-black/25">
        <table className="min-w-[760px] w-full border-collapse text-left text-xs">
          <thead className="bg-panel2 font-mono text-[10px] uppercase tracking-normal text-white/58">
    ...
    <Panel pad="p-3">
      ...
      <div className="grid gap-2">
        {rows.map((row) => {
          ...
          return (
            <button key={row.period} onClick={() => onInspect(metric)} className="rounded-xl border border-white/[0.07] bg-white/[0.025] p-3 text-left hover:bg-white/[0.045] focus-ring">
...
function OperatingKpiTerminal({ data, onInspect }) {
  ...
  return (
    <Panel pad="p-3">
      ...
      <div className="grid gap-3 xl:grid-cols-3">
        {modules.map((mod) => <button key={mod.title} onClick={() => onInspect(inspectPayload({ title: mod.title, value: mod.rows[0][1], formula: mod.formula, source: mod.source, inputs: mod.rows.map((row) => `${row[0]}: ${row[1]} · ${row[2]}`) }))} className="rounded-xl border border-white/[0.075] bg-white/[0.025] p-3 text-left hover:bg-white/[0.045] focus-ring">
...
function CompactPacketPreview({ packet }) {
  ...
  return (
    <div className="mt-4 rounded-xl border border-white/[0.07] bg-white/[0.02] p-3">
      ...
      <Source>{packet.source}</Source>
      <button onClick={() => setHash(packet.hash)} className="mt-3 min-h-10 rounded-lg bg-spacex px-3 text-xs font-[650] text-void hover:bg-white focus-ring">Open packet</button>
    </div>
  )
}
...
function FinancialMetricInspector({ metric, compact = false, onClose }) {
  return (
    <Panel pad="p-3" className={cn(compact ? 'max-h-[78svh] overflow-auto' : 'sticky top-[72px] self-start xl:max-h-[calc(100svh-96px)] xl:overflow-auto')}>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {metric?.packet && <button onClick={() => setHash(metric.packet.hash)} className="inline-flex min-h-11 items-center gap-2 rounded-lg bg-spacex px-3 text-xs font-[650] text-void hover:bg-white focus-ring">Open packet <ChevronRight size={14} /></button>}
        <button onClick={() => setHash('/atlas', { type: 'financial', q: metric?.title || '' })} className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-white/10 bg-white/[0.035] px-3 text-xs text-white/74 hover:bg-white/[0.06] focus-ring"><Search size={14} /> Find in Atlas</button>
      </div>
        ...
        <div className="grid gap-3">
          ...
          <Panel pad="p-3">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <button onClick={() => setShowAudit((v) => !v)} className="min-h-11 rounded-lg border border-white/10 bg-white/[0.035] px-4 text-xs font-[650] text-white/76 hover:bg-white/[0.06] focus-ring">{showAudit ? 'Hide audit tables' : 'Show audit tables'}</button>
            </div>
    ...
    <Section dense eyebrow="Risk radar" title="Actual risk factors, linkable by theme" aside="No invented risk score: this is structure, grouping, search and source packets.">
      <div className="grid gap-3 xl:grid-cols-[300px_1fr]">
        <Panel pad="p-3" className="self-start">
          ...
          <div className="mt-3 space-y-2">{groups.map((g) => <button key={g} onClick={() => { setGroup(g); update({ group: g }) }} className={cn('flex min-h-11 w-full justify-between rounded-lg border px-3 py-2 text-left text-xs focus-ring', group === g ? 'border-red/40 bg-red/10 text-white/86' : 'border-white/[0.07] text-white/58 hover:bg-white/[0.035]')}><span>{g === 'all' ? 'All risk themes' : g}</span><span className="font-mono">{g === 'all' ? grouped.length : grouped.filter((r) => r.theme === g).length}</span></button>)}</div>
        </Panel>
        <div className="grid gap-3">
          ...
          <DataTable maxHeight="max-h-[520px]" empty="No matching risk factors. Try a broader term or clear the theme filter." columns={[
            ...
            { key: 'heading', label: `Risk factor (${filtered.length})`, render: (r) => <button onClick={() => r.packet && setHash(r.packet.hash)} className="text-left text-[13px] leading-5 text-white/76 hover:text-spacex focus-ring">{r.heading}</button> },
          ]} rows={filtered} mobileCard={(r) => (
            <button onClick={() => r.packet && setHash(r.packet.hash)} className="block min-h-16 w-full rounded-lg border border-white/[0.07] bg-white/[0.025] p-3 text-left focus-ring">
...
function SourceTextReader({ current, snippetInfo, query }) {
  ...
  return (
    <div className="rounded-xl border border-white/[0.08] card-surface-soft shadow-[inset_0_1px_0_rgba(255,255,255,.04)]">
      ...
      <div
        ...
        className="max-h-[64svh] overflow-auto rounded-b-xl bg-panel focus-ring md:max-h-[760px]"
      >
      ...
      <div className="grid gap-3 xl:grid-cols-[360px_1fr]">
        ...
        <Panel>
          ...
          <div className="mb-3 flex flex-wrap gap-2">
            <button onClick={() => current && copyText(`SRC · ${current.label}`, () => {})} className="min-h-11 rounded-lg border border-white/10 bg-white/[0.035] px-3 text-xs text-white/70 focus-ring">Copy source label</button>
            <button onClick={() => snippetInfo?.text && copyText(snippetInfo.text, () => {})} disabled={!snippetInfo?.text} className="min-h-11 rounded-lg border border-white/10 bg-white/[0.035] px-3 text-xs text-white/70 disabled:cursor-not-allowed disabled:opacity-40 focus-ring">Copy visible text</button>
          </div>
...
function PosterMode({ data, route }) {
  ...
  return (
    <Section dense eyebrow="Poster mode" title={posterLabel} aside="Designed for clean link cards and manual image posts. Mobile gets a vertical card; desktop keeps the 1200×630 export preview.">
      <div className="mb-3 flex flex-wrap gap-2">{POSTERS.map((p) => <button key={p.id} onClick={() => setHash(`/poster/${p.id}`)} className={cn('min-h-11 rounded-lg border px-3 py-2 text-xs focus-ring', variant === p.id ? 'border-spacex/35 bg-white/[0.07] text-spacex' : 'border-white/[0.08] bg-white/[0.025] text-white/62 hover:bg-white/[0.04]')}>{p.label}</button>)}</div>
